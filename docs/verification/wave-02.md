...
## B01 / B03 / B02 launch contracts — P1 local rows (wave 04)

The user asked for these to be closed locally while the runtime-dependent rows stay blocked. Each is S-evidence with a local oracle; the DAG lists B01's predecessor as S52 and B03's as S18. S18 is accepted, so B03 is in order. B01 and B02 sit behind S52, which is blocked on the runtime — an owner decision was taken to implement them now on the grounds that their own content (per-repo credentials, delivery scheduling) depends on no runtime evidence, and their acceptance is recorded against local oracles only. That deviation is recorded here rather than silently applied.

**B01 — per-repo credentials.** Owner-observed: every repo shares `cfg.githubToken`, so the blast radius of one compromised or over-scoped credential is every repository, and nothing forces a repo's authority to stay with that repo. Envelope: new `repo-policy.ts` + tests, an optional `repos[*].githubToken` validated like the other secrets, a `token` override on `mapping.ts`'s `fetchItem`, and `server.ts` resolving the lookup through the policy. The decisive oracle is a two-repo test proving each repo's requests carry that repo's own token and that one repo's grant is refused for another; a repo without its own token is marked `tokenSource: "shared"` so the weaker posture is visible rather than assumed.

**B03 — honour Retry-After.** Owner-observed: `fetchItem` uses a plain `fetch`, so a 429 fails immediately and the router quietly falls back to the admin channel — every follow-up during a rate-limit window loses its backlink, and retrying harder would only worsen the limit. Envelope: new `github-client.ts` + tests and `mapping.ts` switching to it. `Retry-After` is parsed in both its seconds and HTTP-date forms, bounded by `maxRetryAfterMs`, retried sequentially up to `maxRetries` and then handed back as the last response; `sleep`/`now` are injected so the test observes the requested delays without waiting on the wall clock, which also proves there is no burst retry. Existing URL-contract, pagination, timeout and trust behaviour must not move.
...
## CI caught two test defects on the S32 push

The push for S32 (`9d36283`) failed on the Linux runner, and both failures are real defects rather than flakes to rerun:

1. **`verifyWorkerClaim > test command that exceeds timeoutMs` timed out after 5 s.** The verifier's default runner spawns `sh -c "<command>"` and, on timeout, kills only the shell. The command's child (`sleep 30`) inherits the stdout/stderr pipes, so with the shell dead the pipe read never finishes and the runner hangs until the child exits — 30 s, far past the test budget. It passed locally because macOS's `sh` execs a simple command in place, so killing the shell killed the process; Linux's `sh` forks and the child survived. Fix required: spawn the command in its own process group and kill the group, and bound the pipe waits so a surviving child can never hold the runner open.
